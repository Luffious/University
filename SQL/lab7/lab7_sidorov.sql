...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Дек 02 2020 г., 00:21
...
-- Процедуры
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `Best_Emp` (IN `data1` DATE, IN `data2` DATE)  BEGIN
SELECT employees.name AS "Имя сотрудника", ROUND(SUM(orders.amount * merch.price), 2) as "Продажи"
FROM employees
INNER JOIN orders on orders.emp_id = employees.emp_id
INNER join merch on orders.merc_id = merch.merc_id
WHERE orders.data BETWEEN data1 and data2
GROUP by employees.emp_id
ORDER by 2 DESC
LIMIT 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Best_Shop` (`a` VARCHAR(20), `b` VARCHAR(20))  BEGIN
SELECT shops.name AS "Название магазина", SUM(ROUND(merch.price * orders.amount,2)) as Продажи
FROM shops 
INNER JOIN orders on orders.shop_id = shops.shop_id
INNER JOIN merch on orders.merc_id = merch.merc_id
WHERE shops.adds = a OR shops.adds = b
GROUP by shops.shop_id
ORDER by 2 DESC
LIMIT 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Itog` (IN `a` VARCHAR(20))  BEGIN
SELECT merch.m_name, "Непопулярный товар"
FROM merch
INNER JOIN orders ON orders.merc_id = merch.merc_id
GROUP BY merch.merc_id 
HAVING SUM(orders.amount) < 3 AND merch.m_name = a 
UNION
SELECT merch.m_name, "Обычный товар"
FROM merch
INNER JOIN orders ON orders.merc_id = merch.merc_id
GROUP BY merch.merc_id
HAVING SUM(orders.amount) IN (3,5) AND merch.m_name = a 
UNION
SELECT merch.m_name AS "Название товара", "Популярный товар"
FROM merch
INNER JOIN orders ON orders.merc_id = merch.merc_id
GROUP BY merch.merc_id
HAVING SUM(orders.amount) > 5 AND merch.m_name = a;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `List_Merch` (IN `a` INT)  BEGIN
SELECT merch.m_name AS "Название товара", SUM(ROUND(merch.price * orders.amount, 2)) AS "Продажи"
FROM merch
INNER JOIN orders on orders.merc_id =  merch.merc_id
GROUP by merch.merc_id 
HAVING SUM(ROUND(merch.price * orders.amount, 2)) > a;
END$$
...
--
DELIMITER $$
CREATE TRIGGER `Before_Delete_Merch` BEFORE DELETE ON `merch` FOR EACH ROW DELETE 
FROM orders 
WHERE orders.merc_id = OLD.merc_id
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `Before_Update_Price` BEFORE UPDATE ON `merch` FOR EACH ROW SET NEW.price = NEW.price * 1.2
$$
DELIMITER ;
